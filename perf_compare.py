...
# ── 歷史市場事件對照表 ────────────────────────────────────────────
MARKET_EVENTS = [
    ("1997-07-01", "1998-02-28", "亞洲金融風暴"),
    ("1999-09-21", "1999-10-31", "921大地震（台灣）"),
    ("2000-03-01", "2002-10-31", "網路泡沫破裂"),
    ("2003-01-01", "2003-06-30", "SARS"),
    ("2004-03-19", "2004-05-31", "319槍擊事件"),
    ("2007-10-01", "2009-03-31", "全球金融海嘯"),
    ("2010-04-01", "2010-07-31", "歐債危機（希臘）"),
    ("2011-07-01", "2011-10-31", "美債降評／歐債危機"),
    ("2013-05-01", "2013-07-31", "Taper Tantrum"),
    ("2015-06-01", "2015-10-31", "中國股災"),
    ("2018-01-01", "2018-04-30", "中美貿易戰開打"),
    ("2018-09-01", "2018-12-31", "Fed升息恐慌"),
    ("2020-01-20", "2020-04-30", "COVID-19"),
    ("2021-11-01", "2021-12-31", "Omicron變種"),
    ("2022-01-01", "2022-10-31", "Fed暴力升息／通膨"),
    ("2023-03-01", "2023-04-30", "矽谷銀行危機"),
    ("2024-04-01", "2024-05-31", "地緣政治升溫"),
    ("2024-07-15", "2024-08-20", "日圓升值／套利平倉"),
    ("2025-04-01", "2025-05-15", "川普關稅衝擊"),
]


def label_event(peak_date, trough_date) -> str:
    """將 MDD 高峰～低谷期間與已知市場事件比對，回傳對應標籤。"""
    ps = pd.Timestamp(peak_date)
    pt = pd.Timestamp(trough_date)
    matched = [
        name for s, e, name in MARKET_EVENTS
        if ps <= pd.Timestamp(e) and pt >= pd.Timestamp(s)
    ]
        ...
        bubble_fig.add_trace(go.Scatter(
            ...
            hovertext=[
                (f"{label}<br>回撤: {ev['回撤%']:.1f}%<br>"
                 f"事件: {label_event(ev['高峰日'], ev['低谷日'])}<br>"
